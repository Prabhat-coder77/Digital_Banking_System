...
public class FraudDetectionService {
    ...
    @Value("${fraud.max-transactions-per-minutes}")
    ...
    public void checkTransaction(Map<String, Object> payload) {
        ...
        // Real Balance Fetch From Account Service
    ...
    private FraudCheckResult performFraudChecks(String accountNumber, BigDecimal amount, BigDecimal senderBalance) {
        ...
        {
            return new FraudCheckResult(true,"Too many transactions in 60 seconds"+"- Velocity limit exceed");
        }
        //pattern 2 : amount check
        if(isAmountSuspicious(accountNumber,amount)){
            return new FraudCheckResult(true,"Unusual transactions amount"+"- exceed 3x your average");
        }
        // pattern 3 : Balance Check
        if(senderBalance.compareTo(BigDecimal.ZERO)>0 && isBalanceCheckFailed(senderBalance,amount)){
            return new FraudCheckResult(true,"Transaction Exceed 90% of your account");
        }
    ...
    private boolean isAmountSuspicious(String  accountNumber, BigDecimal amount){
           ...
           log.info("Amount check - amount: {} threshold: {} suspicious: {}",amount,threshold,amount.compareTo(threshold));
